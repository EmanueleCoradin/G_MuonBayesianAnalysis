...
plotdata(data_lifetime, 'lifetime', 'log', plot_histbar = FALSE)
plotdata(data_precession, 'precession', 'log', plot_histbar = FALSE)
```
```{r }
prior_moments[['tau_mean']]
```
```{r }
...
posterior_sample <- coda.samples(model, variable.names = c('N','c','tau'), n.iter = Nrep, thin = thinning)
...
", PriorParam$delta["alpha"], PriorParam$delta["beta"], PriorParam$omega["alpha"], PriorParam$omega["beta"],  PriorParam$alpha["alpha"], PriorParam$alpha["beta"]
, PriorParam$tau["alpha"], PriorParam$tau["beta"])

dataList = list(x = x, y = y, tau = parms['tau'])
model <- jags.model(file = textConnection(model_precession), data = dataList)
adapt(model,n_burnin)
update(model, n.iter = n_burnin)

posterior_sample <- coda.samples(model, variable.names = c('N','c','delta','omega','alpha'), n.iter = Nrep, thin = thinning)
summary(posterior_sample)
...
precession_law <- function(x, parms) {
  parms['N'] * exp(-x / parms['tau']) * (1 + parms['alpha'] * cos(parms['omega'] * x + parms['delta'])) 
  + parms['c']
}

parms_precession <- summary(posterior_sample)$statistics[,'Mean']
parms_precession <- c(parms_precession, parms['tau'])
